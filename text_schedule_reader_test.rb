...
class TextScheduleReaderTest < MiniTest::Test
  def test_initialize
    assert_raises(IOError) {TextScheduleReader.new.read_schedule("non_existant_file_name.boo")}
    assert TextScheduleReader.new.read_schedule('simple.txt')
  end

  def test_schedule_data
    x = TextScheduleReader.new
    x.read_schedule('simple.txt')
