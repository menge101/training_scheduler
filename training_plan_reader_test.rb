...
class TrainingPlanReaderTest < MiniTest::Test
  def test_read_schedule
    assert_raises(IOError) {TrainingPlanReader.new.read_file('non_existant_file_name.boo')}
    assert_raises(NotImplementedError) {TrainingPlanReader.new.read_file('simple.json')}
  end
